$(document).ready(function() {
    ...
    const fetchMedia = async (type, isPreview = false) => {
        ...
        if (type === 'movie') {
            ...
            url = `https://api.themoviedb.org/3/discover/tv?api_key=${config.apiKey}&with_genres=16&sort_by=first_air_date.desc&with_original_language=ja&vote_average.gte=7&vote_count.gte=25&without_keywords=10121,9706,264386,280003,158718`;
            mediaType = 'tv';
        } else if (type === 'kdrama') {
            ...
            url = `https://api.themoviedb.org/3/discover/tv?api_key=${config.apiKey}&with_original_language=zh&sort_by=first_air_date.desc&vote_average.gte=7&vote_count.gte=25&without_genres=16,10759,10765,10768&without_keywords=15060,248451,289844,12995,195013,184656,234890`;
            mediaType = 'tv';
